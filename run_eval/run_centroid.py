# run_centroid.py
"""
用 Centroid + Hungarian tracker 在 CanolaTrack val 集上推理。
结果保存到 outputs/centroid/tracks/，可直接用 run_eval.py 评估。
"""
...
OUTPUT_DIR = "outputs/centroid/tracks"
...
for plant, keys in tqdm(plants.items(), desc="Plants"):
    ...
    tracker = CentroidTracker(max_age=5, max_dist=200)
    lines = []
    for k in keys:
